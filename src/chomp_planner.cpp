...
class CHOMPPlanner : public rclcpp::Node
{
public:
    CHOMPPlanner() : Node("chomp_planner"), joint_model_group(nullptr) {}
    ...
    void executeMotionPlan()
    {
        ...
        // move_group_interface->setPoseTarget(target_pose); // Because CHOMP planner needs a joint-space goal

        // Before planning, verify that I indeed use CHOMP for motion planning
        RCLCPP_INFO_STREAM(this->get_logger(), "Current planning pipeline: " << move_group_interface->getPlanningPipelineId());
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<CHOMPPlanner>();
    node->initialize();
